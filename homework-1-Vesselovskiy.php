...
function qInfo()
{
    echo "Если что-то пошло не так, пример запроса на сложение числа 1 и 2 выглядит так: ?a=1&b=2&c=+ <br/>";
...
if ($a != 0)
    switch ($a) {
        case $a == "empty":
            echo "Неверный запрос, введи в запросе первую переменную </br></br>";
            $stop = true;
            qInfo();
            break;
        case !is_numeric($a):
...
if ($b != 0)
    if ($stop == false) {
        switch ($b) {
            case $b == "empty":
                echo "Неверный запрос, введи в запросе вторую переменную </br></br>";
                $stop = true;
                qInfo();
                break;
            case !is_numeric($b):
        ...
        switch ($c) {
            case "0":
                echo "Неверный запрос, введи в запросе оператор который я понимаю </br></br>";
                $stop = true;
                qInfo();
                break;
